...
public:
    ...
    {
        vector<int> ans;
        vector<int> XORl(arr.size(), 0);
        vector<int> XORr(arr.size(), 0);
        XORl[0] = arr[0];
        XORr[arr.size()-1] = arr[arr.size()-1];
        int totalXOR = arr[0];
        
        for(int i = 1; i < arr.size(); ++i)
        {
            XORl[i] = XORl[i-1] ^ arr[i];
            totalXOR ^= arr[i];
        }
        ...
        for(int i = 0; i < queries.size(); ++i)
        {
            ...
            if(l == 0 && r == arr.size()-1)
                ans.push_back(XORl[r]);
            else if(l == 0)
                ans.push_back(XORl[r]);
            else if(r == arr.size()-1)
                ans.push_back(XORr[l]);
            else
                ans.push_back(totalXOR ^ XORl[l-1] ^ XORr[r+1]);
        }
        return ans;
    }
};
