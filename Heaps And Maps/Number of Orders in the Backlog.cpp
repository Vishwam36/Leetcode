...
// Easy concept, complicated implementation

/*
You are given a 2D integer array orders, where each orders[i] = [pricei, amounti, orderTypei] denotes that amounti orders have been placed of type orderTypei 
...
smaller than or equal to the current buy order's price, they will match and be executed, and that sell order will be removed from the backlog. 
...
- 4 orders of type buy with price 30 are placed. The first 2 orders are matched with the 2 sell orders of the least price, which is 15 and these 2 sell orders are removed 
from the backlog. The 3rd order is matched with the sell order of the least price, which is 25 and this sell order is removed from the backlog. 
    ...
    int getNumberOfBacklogOrders(vector<vector<int>>& orders) 
    {
        ...
        for(int i = 0; i < orders.size(); ++i)
        {
            ...
            if(type == 0)
            {
                ...
                while(!sell.empty() && sell.top()[0] <= price && amount)
                {
                    vector<int> o = sell.top();
                    sell.pop();
                    if(o[1] > amount)
                    {
                        o[1] -= amount;
            ...
            else
            {
                ...
                while(!buy.empty() && buy.top()[0] >= price && amount)
                {
                    vector<int> o = buy.top();
                    buy.pop();
                    if(o[1] > amount)
                    {
                        o[1] -= amount;
