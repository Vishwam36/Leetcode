...
/*
You are given a 0-indexed 2D array grid of size 2 x n, where grid[r][c] represents the number of points at position (r, c) on the matrix. Two robots are playing a 
game on this matrix.
...
// Note :
// robot-1 wants to minimize robot-2s score
// It doesnt mean that robot-1s score will be maximized
...
public:
    long long gridGame(vector<vector<int>>& grid) 
    {
        ...
        for(int i = 1; i < n; ++i)
        {
            pre[0][i] = (long long)grid[0][i] + pre[0][i-1];
            pre[1][i] = (long long)grid[1][i] + pre[1][i-1];
        }
        
        long long ans = pre[0].back()-pre[0][0];
        for(int i = 1; i < n; ++i)
            ans = min(ans, max(pre[0].back()-pre[0][i], pre[1][i-1]));
