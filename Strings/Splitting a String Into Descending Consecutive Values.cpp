...
For example, the string s = "0090089" can be split into ["0090", "089"] with numerical values [90,89]. The values are in descending order 
and adjacent values differ by 1, so this way is valid.
...
public:
    ...
    {
        vector<long long> v;
        long long num = stoll(s.substr(0, len));
        v.push_back(num);
        string temp;
        for(int i = len; i < s.size(); ++i)
        {
            temp += s[i];
            if(temp.size() <= 17 && stoll(temp) >= num)
                break;
            
            if(temp.size() <= 17 && num-stoll(temp) == 1)
            {
                num = stoll(temp);
                temp = "";
                v.push_back(num);
    ...
    bool splitString(string s) 
    {
        ...
        for(int i = 1; i < min(11, (int)s.size()); ++i)
            ans = ans || check(i, s);
