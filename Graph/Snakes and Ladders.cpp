...
you do not continue moving.  (For example, if the board is `[[4,-1],[-1,3]]`, and on the first move your destination square is `2`, then you finish 
        ...
        while(!q.empty())
        {
            ...
            for(int l = 0; l < len; ++l)
            {
                ...
                for(int k = 1; k <= 6 && curr+k <= n*n; ++k)
                {
                    ...
                    // finding row and column of number
                    int i = (n-1) - ((curr+k)%n == 0 ? (curr+k)/n - 1 : (curr+k)/n);
                    int j = (n-1-i) % 2 == 0 ? ((curr+k)-1) % n : (n-1) - ((curr+k)-1) % n;
