...
you must take n trains in sequential order. You are also given an integer array dist of length n, 
...
class Solution {
public:    
    double find_time(int mid, vector<int>& dist)
    {
        double t = 0;
        for(int i = 0; i < dist.size()-1; ++i)
            t += dist[i] % mid == 0 ? dist[i]/mid : (dist[i]/mid)+1;
    
        t += (double)dist.back()/mid;
        return t;
    }
